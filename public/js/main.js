...
(() => {
  ...
  function createParticleField() {
    ...
    for (let i = 0; i < CONFIG.particleCount; i++) {
      ...
      positions[i * 3 + 2] =
        (Math.random() - 0.5) * CONFIG.particleFieldSize * 0.5;
  ...
  function animate() {
    ...
    const driftX =
      Math.sin(time * CONFIG.cameraDriftSpeed * 1000) *
      CONFIG.cameraDriftRadius;
    const driftY =
      Math.cos(time * CONFIG.cameraDriftSpeed * 800) *
      CONFIG.cameraDriftRadius *
      0.6;
    ...
    nodes.forEach((node) => {
      ...
      node.position.x =
        ud.originalPos.x + Math.sin(time * ud.speed + ud.phase) * ud.amplitude;
      node.position.y =
        ud.originalPos.y +
        Math.cos(time * ud.speed * 0.7 + ud.phase) * ud.amplitude * 0.7 +
        scrollEffect;
      node.position.z =
        ud.originalPos.z + Math.sin(time * ud.speed * 0.35 + ud.phase) * 6;
    ...
    connections.forEach((conn) => {
      ...
      const pulse =
        0.7 + 0.3 * Math.sin(time * 2 + conn.userData.startIdx * 0.08);
      conn.material.opacity =
        conn.userData.baseOpacity * pulse * (1 + scrollNorm * 0.6);
    });
    ...
    if (signals.length > 0 && connections.length > 0) {
      ...
      signals.forEach((signal) => {
        signal.progress +=
          signal.speed *
          signal.direction *
          (1 + scrollNorm * CONFIG.scrollMultiplier);
        ...
        if (conn) {
          ...
          positions[signal.index * 3] =
            start.position.x +
            (end.position.x - start.position.x) * signal.progress;
          positions[signal.index * 3 + 1] =
            start.position.y +
            (end.position.y - start.position.y) * signal.progress;
          positions[signal.index * 3 + 2] =
            start.position.z +
            (end.position.z - start.position.z) * signal.progress;
        }
      });
    ...
    energyWaves.forEach((wave, i) => {
      ...
      const scale =
        ud.baseScale +
        Math.sin(time * ud.speed + ud.phase) * 0.35 +
        scrollNorm * 0.5;
      wave.scale.set(scale, scale, 1);
      wave.rotation.z = time * 0.1 * (i % 2 === 0 ? 1 : -1);
      wave.material.opacity =
        (0.05 - i * 0.012) * (1 + Math.sin(time * 2) * 0.35);
    });
